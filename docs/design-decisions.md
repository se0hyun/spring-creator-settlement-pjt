...
**`endAt`에 `null` 대신 원거리 날짜 사용**: 현재 유효한 수수료율 조회 쿼리가 `f.endAt >= :now` 조건을 사용합니다. JPQL에서 `null` 비교는 `UNKNOWN`을 반환하여 해당 레코드가 조회되지 않습니다. "종료일 없음(무기한)"을 표현하기 위해 null 대신 충분히 먼 미래 날짜를 사용합니다. 샘플 데이터에서는 `2099-12-31 23:59:59` 등으로 설정했습니다.
...
## 10. 수수료 구간 변경 API 및 스냅샷 일관성

- 관리자가 `POST /fee-rates`로 `effectiveFrom`부터 새 수수료 구간을 등록하면, 그 시각과 겹치던 **직전 `FeeRecord`의 종료 시각**을 새 구간 시작 직전으로 자동 조정하여 행이 겹치지 않게 한다.
- **월별 정산**과 **기간 집계**에서 플랫폼 수수료 비율은 각 **월 시작 시점**에 유효한 `FeeRecord` 한 건으로 결정된다 (`findActiveFeeRate`).
- **구매**: `SaleRecord.feeRate`는 결제 의미 시각인 **`paidAt`** 기준 유효 수수료를 스냅샷한다 (서버 시각 `now()`가 아님).
- **취소**: `CancelRecord.feeRate`는 **`canceledAt`** 기준 유효 수수료를 스냅샷한다. 월별 순매출·정산 금액 계산은 금액 합산 로직은 동일하고, 스냅샷은 표시·감사 추적용이다.
